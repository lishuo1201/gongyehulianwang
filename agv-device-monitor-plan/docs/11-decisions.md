...
| JUnit Jupiter | `6.0.3` | Boot BOM 和 Surefire/Failsafe 报告 | 5 个测试通过，0 失败、0 错误、0 跳过 |
| Compiler / Surefire / Failsafe | `3.15.0` / `3.5.6` / `3.5.6` | 继承固定 Boot 父 POM 的插件版本 | Java 17 编译及 `*Test`、`*IT` 分阶段执行通过 |
| MySQL | 本地官方镜像版本 `8.4.10-1.el9`，固定 digest 见下方 | 只读 `docker image inspect mysql:8.4` | 仅核对本地镜像元数据；未启动数据库，T02 验证迁移 |
| MySQL JDBC / Flyway / Testcontainers | BOM 管理 `9.7.0` / `12.4.0` / `2.0.5` | 已读取 Boot BOM；应用尚未引入这些依赖 | T02 验证依赖、MySQL 8.4 兼容性和迁移后采用 |
...
MySQL 后续测试/部署使用已核对的不可变引用：`mysql:8.4.10@sha256:8dbcf531a03aade657e181b9cf2f1d1803ce621a1d55610cb44cb531ab7d7db6`。此处锁定镜像身份，不表示数据库已经运行或迁移通过。
