...
状态：下方Demo业务架构已实现；工程验收和学习进度见 [任务交接](06-tasks.md)。默认参数见 [需求](01-requirements.md)，协议见 [寄存器表](10-modbus-register-map.md)。
...
| `lastFreshAt` | 首次有效样本，或 heartbeat 与最后成功提交的业务基准不同；随整个事务成功后推进 |
...
- 通信计时与数据库成功提交分开：正常响应立即保留通信事实，数据库失败不能被当成设备通信失联。
- 业务状态在副本上计算；完整事务成功后才替换内存业务基准。失败候选丢弃，健康扫描不得仅补写失败样本的快照而遗漏历史或告警；下一次相同心跳仍可重试。
- 重启和启停保留旧可信值及展示时间，独立重建本进程单调时钟基准。首次有效响应不与重启前的心跳去重。
- 独立线程在同一读取事务内获取快照与活动告警，事务返回后替换不可变视图；SSE 取缓存不等待数据库。刷新失败和持久化失败分别标记降级。
- 唯一冲突只对已识别的活动槽进行一次完整回滚后的重试；内部写入不嵌套重试。同一历史 sample_key 重复交付不再写快照、历史或事件。
